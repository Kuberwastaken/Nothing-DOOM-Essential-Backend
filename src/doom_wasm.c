...
#include "doom_wasm.h"
#include "file_embedded_in_code/doom1-silent.wad.h"
...
struct DB_BytesForAllWads DG_GetWads() {
  ...
  if (numberOfWads > 0) {
    ...
    printf("Defaulting to loading Doom shareware WAD because no WAD data was "
           "provided\n");
    result.iWad.data = malloc(doom1_silent_wad_length);
    memcpy(result.iWad.data, doom1_silent_wad_data, doom1_silent_wad_length);
    result.iWad.byteLength = doom1_silent_wad_length;
