...
EMBEDDED_BINARY_FILES = doom1-silent.wad
...
# Use the provided doom1-silent.wad file
$(OUTPUT_DIR)/doom1-silent.wad: | $(OUTPUT_DIR)
	@echo [Copying local doom1-silent.wad]
	$(VB)cp doom1-silent.wad $(OUTPUT_DIR)/doom1-silent.wad
...
$(FILE_EMBEDDED_IN_CODE_DIR)/%.c $(FILE_EMBEDDED_IN_CODE_DIR)/%.h: $(OUTPUT_DIR)/% utils/generate_code_for_embedded_file.py | $(FILE_EMBEDDED_IN_CODE_DIR)
	...
	${VB}python3 utils/generate_code_for_embedded_file.py --input $< --destination-folder $(FILE_EMBEDDED_IN_CODE_DIR)
